...
export default function ContextPanel({ state }: ContextPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-paper-3 mb-3">
          Your analysis
        </div>
        <div className="border border-wire rounded-xl overflow-hidden bg-ink-1">
          {[
            ...
            <div key={label} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-wire' : ''}`}>
              <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-paper-3 w-20 shrink-0">
                {label}
              </span>
              <span className={`text-[13px] font-medium truncate ${value ? 'text-paper' : 'text-paper-3'}`}>
                {value ?? '—'}
              </span>
            </div>
          ))}
      ...
      {hints.length > 0 && (
        <div>
          ...
          <div className="flex flex-col gap-2">
            {hints.map(h => (
              <div key={h} className="flex items-center gap-2.5 text-[13px] text-paper-2">
                <span className="w-1 h-1 rounded-full bg-acid shrink-0" />
                {h}
              </div>
            ))}
          </div>
      ...
      <div className="border-t border-wire pt-5">
        <div className="flex flex-col gap-1.5 text-[12px] text-paper-3">
      ...
      <div className={`text-[12px] font-semibold text-acid transition-opacity duration-300 ${hasAny ? 'opacity-100' : 'opacity-0'}`}>
